...
pub async fn embed_metadata(file_path: &str, metadata: ImageMetadata) -> AppResult<String> {
    ...
    if output_path.exists() {
        log::info!("Output file already exists, attempting to remove: {}", output_path.display());
    ...
    log::info!("Attempting to save PNG with metadata to: {}", output_path.display());
    ...
    log::info!("Embedded metadata in {} -> {}", file_path, output_path.display());
...
fn create_vrchat_metadata_json(metadata: &ImageMetadata) -> AppResult<String> {
    ...
    json_obj.insert("application".to_string(), serde_json::Value::String("VRChat Photo Uploader".to_string()));
    json_obj.insert("version".to_string(), serde_json::Value::Number(2.into()));
    json_obj.insert("created_at".to_string(), serde_json::Value::String(
    ...
    if let Some(ref author) = metadata.author {
        ...
        author_obj.insert("displayName".to_string(), serde_json::Value::String(author.display_name.clone()));
        author_obj.insert("id".to_string(), serde_json::Value::String(author.id.clone()));
    ...
    if let Some(ref world) = metadata.world {
        ...
        world_obj.insert("name".to_string(), serde_json::Value::String(world.name.clone()));
        world_obj.insert("id".to_string(), serde_json::Value::String(world.id.clone()));
        world_obj.insert("instanceId".to_string(), serde_json::Value::String(world.instance_id.clone()));
        json_obj.insert("world".to_string(), serde_json::Value::Object(world_obj));
    }
    
    // Add players array
    let players_array: Vec<serde_json::Value> = metadata.players
        .iter()
        .map(|player| {
            let mut player_obj = serde_json::Map::new();
            player_obj.insert("displayName".to_string(), serde_json::Value::String(player.display_name.clone()));
            player_obj.insert("id".to_string(), serde_json::Value::String(player.id.clone()));
            serde_json::Value::Object(player_obj)
        })
        .collect();
    
    json_obj.insert("players".to_string(), serde_json::Value::Array(players_array));
    
    let json_value = serde_json::Value::Object(json_obj);
    Ok(serde_json::to_string_pretty(&json_value)?)
}

fn save_png_with_metadata(img: &image::DynamicImage, output_path: &Path, metadata_json: &str) -> AppResult<()> {
    use std::io::Cursor;
...
fn insert_text_chunk(result: &mut Vec<u8>, keyword: &str, text: &str) -> AppResult<()> {
    ...
    if keyword.is_empty() || keyword.len() > 79 {
        return Err(AppError::validation("keyword", "Keyword must be 1-79 bytes"));
    }
...
pub async fn extract_metadata_from_png(file_path: &str) -> AppResult<Option<String>> {
    InputValidator::validate_image_file(file_path)?;
    
    match crate::image_processor::extract_metadata(file_path).await {
        Ok(Some(_metadata)) => Ok(Some("Metadata found and parsed successfully".to_string())),
        Ok(None) => Ok(None),
        Err(e) => Err(e),
    }
}

// Utility function to validate metadata before embedding
pub fn validate_metadata(metadata: &ImageMetadata) -> AppResult<()> {
    // Check author info
    if let Some(ref author) = metadata.author {
        if author.display_name.trim().is_empty() {
            return Err(AppError::validation("author.display_name", "Display name cannot be empty"));
        }
        if author.id.trim().is_empty() {
            return Err(AppError::validation("author.id", "User ID cannot be empty"));
        }
    }
    ...
    if let Some(ref world) = metadata.world {
        if world.name.trim().is_empty() {
            return Err(AppError::validation("world.name", "World name cannot be empty"));
        }
        if world.id.trim().is_empty() {
            return Err(AppError::validation("world.id", "World ID cannot be empty"));
        }
    }
    
    // Check players
    for (i, player) in metadata.players.iter().enumerate() {
        if player.display_name.trim().is_empty() {
            return Err(AppError::validation(
                &format!("players[{}].display_name", i), 
