...
use crate::errors::{ProgressState, safe_progress_update, safe_progress_read};
...
pub fn is_session_cancelled(progress_state: &ProgressState, session_id: &str) -> bool {
    safe_progress_read(progress_state, session_id, "cancellation check", |progress| {
...
pub fn mark_session_cancelled(progress_state: &ProgressState, session_id: &str) {
    safe_progress_update(progress_state, session_id, "mark cancelled", |progress| {
        ...
        log::info!("Marked session {} as cancelled with {} completed uploads", 
            session_id, progress.completed);
    });
}

/// Update progress to show current file being processed
pub fn update_progress_current(progress_state: &ProgressState, session_id: &str, file_path: String) {
    safe_progress_update(progress_state, session_id, "current file update", |progress| {
...
pub fn update_progress_current_with_phase(
    ...
    safe_progress_update(progress_state, session_id, "phase update", |progress| {
        ...
        log::debug!("Progress: {} {} ({}%)", phase, file_path, progress_percent as u32);
    });
}

/// Mark a file upload as successful
pub fn update_progress_success(progress_state: &ProgressState, session_id: &str, file_path: String) {
    safe_progress_update(progress_state, session_id, "success update", |progress| {
        ...
        log::info!("Progress: Successfully uploaded {} ({}/{})", 
            file_path, progress.completed, progress.total_images);
    });
}
...
pub fn update_progress_failure(
    ...
    safe_progress_update(progress_state, session_id, "failure update", |progress| {
        ...
        if let Some(existing_failure) = progress.failed_uploads.iter_mut().find(|f| f.file_path == file_path) {
        ...
        log::warn!("Progress: Failed to upload {} - {} ({}/{})", 
            file_path, error, progress.completed, progress.total_images);
    });
}
...
pub fn update_progress_group_failure(
    ...
    safe_progress_update(progress_state, session_id, "group failure update", |progress| {
        progress.completed += 1;
...
pub fn update_time_estimate(
    ...
    safe_progress_update(progress_state, session_id, "time estimate update", |progress| {
...
pub fn mark_session_failed(progress_state: &ProgressState, session_id: &str) {
    safe_progress_update(progress_state, session_id, "mark failed", |progress| {
        ...
        log::error!("Session {} marked as failed: {}/{} successful, {} failed", 
            session_id, 
            progress.successful_uploads.len(),
            progress.total_images,
            progress.failed_uploads.len()
