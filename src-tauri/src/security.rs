...
mod tests {
    ...
    fn test_validate_webhook_name_valid() {
        ...
        for name in valid_names {
            assert!(InputValidator::validate_webhook_name(name).is_ok(), 
    ...
    fn test_validate_webhook_name_invalid() {
        ...
        for (name, reason) in invalid_names {
            assert!(InputValidator::validate_webhook_name(name).is_err(), 
                   "Invalid name '{}' should fail validation ({})", name, reason);
        }
    }
    ...
    fn test_validate_webhook_url_valid() {
        ...
        for url in valid_urls {
            assert!(InputValidator::validate_webhook_url(url).is_ok(), 
    ...
    fn test_validate_webhook_url_invalid() {
        ...
        for (url, reason) in invalid_urls {
            assert!(InputValidator::validate_webhook_url(url).is_err(), 
                   "Invalid URL '{}' should fail validation ({})", url, reason);
        }
    }
    ...
    fn test_validate_upload_settings_valid() {
        for i in 1..=10 {
            assert!(InputValidator::validate_upload_settings(i, true).is_ok(),
                   "Max images {} should be valid", i);
            assert!(InputValidator::validate_upload_settings(i, false).is_ok(),
                   "Max images {} should be valid", i);
        }
    }
    ...
    fn test_sanitize_filename() {
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(result, expected, 
                      "Sanitizing '{}' should produce '{}'", input, expected);
        }
    }
    ...
    fn test_validate_file_path_invalid_paths() {
        ...
        for (path, reason) in invalid_paths {
            assert!(InputValidator::validate_file_path(path).is_err(),
                   "Invalid path '{}' should fail validation ({})", path, reason);
        }
    }
    ...
    fn test_validate_image_file_with_temp_file() {
        ...
        let png_data = vec![
            ...
            0x02, 0x00, 0x01, // IDAT data and CRC
